...
function optionsToRequestOptions(options) {
    ...
    if (options.scope) {
        if (options.scope.project) {
            qs.project = Ref.getRelative(options.scope.project);
            ...
            qs.workspace = Ref.getRelative(options.scope.workspace);
        }
    }
...
RestApi.prototype.query = function(options, callback) {
    ...
    var requestOptions = _.merge({
        url: Ref.getRelative(options.ref) || format('/%s', options.type),
        qs: {
            start: options.start,
